...
parse_RNAseq_norm_reads <- function(path) {
  # extract file name
  ...
  counts <- fread(path) %>%
    ...
    rename(!!as.symbol(sample_name) := tpm_unstranded)
...
RNAseq <- map(RNAseq_files, parse_RNAseq_norm_reads) %>%
  reduce(full_join, by=c('gene_id', 'gene_name', 'gene_type'))

# export
fwrite(RNAseq, file = "result/TCGA-HNSC.RNAseq.TPM.tsv", quote = F, sep = "\t")


# Parse miRNAseq -----------------------------------------------------------------------

# Function for parsing read counts
parse_miRNA_norm_reads <- function(path) {
  # extract file name
  file_name <- str_extract(path, "(?<=/)[^/]*$")
  # replace file name with sample id
  sample_name <- filename_to_sampleID[file_name]
  counts <- fread(path) %>%
    select(miRNA = miRNA_ID, reads_per_million_miRNA_mapped) %>%
    # rename the counts column with the sample name
    rename(!!as.symbol(sample_name) := reads_per_million_miRNA_mapped)
...
# parse all read counts and join them in a single table
miRNAseq <- map(miRNAseq_files, parse_miRNA_norm_reads) %>%
  reduce(full_join, by='miRNA')

# export
fwrite(miRNAseq, file = "result/TCGA-HNSC.mirnaRNAseq.RPM.tsv", quote = F, sep = "\t")
...
# Function for parsing read counts
parse_miRNA_isoform_norm_reads <- function(path) {
  # extract file name
  file_name <- str_extract(path, "(?<=/)[^/]*$")
  # replace file name with sample id
  sample_name <- filename_to_sampleID[file_name]
  ...
  counts <- fread(path, data.table = F) %>%
    ...
    summarize(reads_per_million_miRNA_mapped = sum(reads_per_million_miRNA_mapped)) %>%
    # rename the counts column with the sample name
    rename(!!as.symbol(sample_name) := reads_per_million_miRNA_mapped) %>%
    as.data.frame()
...
# parse all read counts and join them in a single table
miRNAseq_isoform <- map(miRNAseq_isoform_files, parse_miRNA_isoform_norm_reads) %>%
...
# export
fwrite(miRNAseq_isoform, file = "result/TCGA-HNSC.miRNAseq_isoform.RPM.tsv", quote = F, sep = "\t")
